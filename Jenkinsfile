node ('centos8') {
    ...
    stage('push') {
      withCredentials([usernameColonPassword(credentialsId: '0f11d4d1-6557-423c-b5ae-693cc87f7b4b', variable: 'HUB_LOGIN')]) {
        ...
        sh 'if [[ "$BRANCH_NAME" == "master" ]] ; then export TAG=latest; else export TAG="$BRANCH_NAME"; fi && podman push --creds "$HUB_LOGIN" ebi2gus docker://docker.io/veupathdb/ebi2gus:"$TAG"'
        }
      }
